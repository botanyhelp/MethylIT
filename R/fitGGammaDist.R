...
#'   parameter.values = list(alpha = 'value', scale = 'value', mu = 'value', psi
#'   = 'value') or if mu = 0, as: parameter.values =list(alpha = 'value', scale
#'   = 'value', psi = 'value'). If not provided, then an initial guess is
#'   provided.
...
fitGGammaDist <- function(x, probability.x, parameter.values,
  ...
  if (inherits( FIT, "try-error") && location.par) {
    k = 1
    messg = paste0("* The 'GGamma' model did not fit the data.\n",
                   "Trying to fit based on approach to 'Gamma3P' model ...")
    message(messg)
    starts <- list(alpha = 1, scale = scale, mu = 0, psi = MEAN^2/VAR)
    while (inherits( FIT, "try-error") && k <= length(psi)) {
      starts$psi = psi[k]
      FIT <- try(nls.lm(par = starts, fn = optFun, probfun = pggamma,
                        quantiles = X, prob = pX,
  ...
  if (inherits( FIT, "try-error")) {
    k = 1
    messg = paste0("* The approach to 'Gamma3P' model did not fit the data.\n",
                   "Trying to fit based on approach to 'Gamms2P' model ...")
    message(messg)
    starts <- list(alpha = 1, scale = scale, psi = MEAN^2/VAR)
    while (inherits( FIT, "try-error") && k <= length(psi)) {
      starts$psi = psi[k]
      FIT <- try(nls.lm(par = starts, fn = optFun, probfun = pggamma,
                        quantiles = X, prob = pX,
