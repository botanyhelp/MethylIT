...
library(MethylIT)
context("MethylIT BICmodel tests")

test_that("BICmodel function test", {
    ...
    nlm <- nls(Y ~ a * exp( b * X), data = data.frame(X = x, Y = y),
                ...
                algorithm = "port")
    # The estimations of Akaike information criteria given by BIC' function from
    # 'stats' R package and from 'BICmodel' function are equals.
    #BICmodel(nlm) == BIC(nlm)
    #all.equal(BICmodel(nlm), BIC(nlm))
    expect_equal(BICmodel(nlm), BIC(nlm))
    # Now, using residuals from the fitted model:
    res = y - coef(nlm)[1] * exp(coef(nlm)[2] * x)
    expect_equal(BICmodel(residuals = res, np = 2), BIC(nlm))
